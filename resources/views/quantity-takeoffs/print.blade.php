...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .data-table .section-title td { background: #f0f4ff; font-weight: bold; font-size: 6px; text-align: left; }
        .data-table .element-row td { font-weight: bold; font-size: 6px; }
...
<body>
    ...
    <div class="sheet">
        <table class="header-table">
            <tr>
                ...
                <td width="30%" class="doc-info">
                    ...
                    <strong>Page:</strong> {{ $sheet->page_no }}
                </td>
            </tr>
        </table>
        ...
        <div class="info-row"><strong>Location:</strong> Addis Ababa &nbsp; <strong>Date:</strong> {{ $sheet->measurement_date->format('d/m/Y') }}</div>
        ...
        <table class="data-table">
            <thead>
                <tr>
                    <th width="3%">No</th><th width="5%">Qty</th><th width="9%">Size (L×W×H)</th>
                    <th width="8%">Product</th><th width="25%">Description</th>
                    <th width="3%">No</th><th width="5%">Qty</th><th width="9%">Size (L×W×H)</th>
                    <th width="8%">Product</th><th width="25%">Description</th>
                </tr>
            </thead>
            <tbody>
                ...
                @foreach($sheet->items as $item)
                    @php
                        $leftDescs = $item->descriptions->where('side', 'left');
                        $rightDescs = $item->descriptions->where('side', 'right');
                    ...
                    @for($i = 0; $i < $maxRows; $i++)
                        @php
                            $leftDesc = $leftDescs->values()->get($i);
                            $rightDesc = $rightDescs->values()->get($i);
                            $leftMeasurements = $leftDesc ? $leftDesc->measurements : collect();
                            $rightMeasurements = $rightDesc ? $rightDesc->measurements : collect();
                            $maxM = max($leftMeasurements->count(), $rightMeasurements->count(), 1);
                        @endphp
                        
                        <!-- Description Headers -->
                        <tr class="desc-header">
                            <td colspan="5" class="text-left">{{ $leftDesc->description ?? '' }}</td>
                            <td colspan="5" class="text-left">{{ $rightDesc->description ?? '' }}</td>
                        </tr>
                        
                        @for($m = 0; $m < $maxM; $m++)
                            @php
                                $lm = $leftMeasurements->values()->get($m);
                                $rm = $rightMeasurements->values()->get($m);
                                if ($lm) { $la = $lm->total_area_volume; $grandTotalAll += $la; }
                                if ($rm) { $ra = $rm->total_area_volume; $grandTotalAll += $ra; }
                            @endphp
                            <tr class="element-row">
                                @if($lm)<td></td><td class="text-right">{{ $lm->quantity_count }}</td><td class="text-right">{{ number_format($lm->length, 2) }}</td><td></td><td></td>@else<td colspan="5"></td>@endif
                                @if($rm)<td></td><td class="text-right">{{ $rm->quantity_count }}</td><td class="text-right">{{ number_format($rm->length, 2) }}</td><td></td><td></td>@else<td colspan="5"></td>@endif
                            </tr>
                            <tr>
                                @if($lm)<td></td><td></td><td class="text-right">{{ number_format($lm->length, 2) }}</td><td></td><td></td>@else<td colspan="5"></td>@endif
                                @if($rm)<td></td><td></td><td class="text-right">{{ number_format($rm->length, 2) }}</td><td></td><td></td>@else<td colspan="5"></td>@endif
                            </tr>
                            <tr>
                                @if($lm)<td></td><td></td><td></td><td class="text-right">{{ number_format($la ?? 0, 2) }}</td><td></td>@else<td colspan="5"></td>@endif
                                @if($rm)<td></td><td></td><td></td><td class="text-right">{{ number_format($ra ?? 0, 2) }}</td><td></td>@else<td colspan="5"></td>@endif
                            </tr>
                            <tr>
                    ...
                    <!-- Item Total -->
                    <tr class="total-row">
                        @php $itemTotal = $item->descriptions->sum(fn($d) => $d->measurements->sum('total_area_volume')); @endphp
        ...
        <table class="signatures">
            <tr>
                <td><div class="sig-name">{{ $sheet->measured_by }}</div><div class="sig-line">Measured By</div><div class="sig-date">{{ $sheet->measurement_date->format('d-m-Y') }}</div></td>
